...
uses
  Classes,  SysUtils, ZConnection, ZDataset,  LazFileUtils, Forms,
  Controls, Graphics,
  Grids,  StdCtrls,  IniPropStorage, ComCtrls,
  DB, Dialogs, ExtCtrls,
  dateutils, DateTimePicker, LazUTF8, Spin, strutils, MouseAndKeyInput,
...
//  timeout_signal=9; //предудпреждение перед закрытием
  ...
  max_transaction_time=8; //кол-во минут максимального выполнения транзакции
  inettick=5;
...
var
  ...
  unused_critical_time:integer=20;//время минут когда можно вычеркнуть билет
  un_critical_time: Tdatetime;//время (мин) когда можно восстановить вычеркнутый билет
  //*******************
...
begin
  ...
  Label3.Caption := datetostr(work_date);  //рабочая дата
...
begin
  ...
  begin
   ...
   If not(Connect2(form1.Zconnection1, 1)) then
     begin
      showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k3-');
      exit;
     end;
...
begin
  ...
  //showmessage(sale_server);
   ...
   If not(Connect2(form1.Zconnection1, 1)) then
     ...
     sale_server:=ConnectINI[14];
     otkuda_name:=server_name;
      showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k4-');
      Close;
      exit;
     end;
 ...
 If filtr=9 then
  ...
  //ZReadOnlyQuery1.SQL.Add('CASE WHEN position(''[u3]'' in a.uslugi_text)>0 ');
  //ZReadOnlyQuery1.SQL.Add('THEN substr(a.uslugi_text,position(''[u3]'' in a.uslugi_text)+5,(position(''|Страх'' in a.uslugi_text))-position(''[u3]'' in a.uslugi_text)-5)::numeric ELSE 0 END as strah_sbor_nal ');
  //ZReadOnlyQuery1.SQL.Add(',CASE WHEN position(''[u4]'' in a.uslugi_text)>0 ');
  //ZReadOnlyQuery1.SQL.Add('THEN substr(a.uslugi_text,position(''[u4]'' in a.uslugi_text)+5,(position(''|Страх'' in a.uslugi_text))-position(''[u4]'' in a.uslugi_text)-5)::numeric ELSE 0 END as strah_sbor_credit ');
  //ZReadOnlyQuery1.SQL.Add(',CASE WHEN position(''[u5]'' in a.uslugi_text)>0 ');
  //ZReadOnlyQuery1.SQL.Add('THEN substr(a.uslugi_text,position(''[u5]'' in a.uslugi_text)+5,(position(''|Коми'' in a.uslugi_text))-position(''[u5]'' in a.uslugi_text)-5)::numeric ELSE 0 END as kom_sbor ');
  //ZReadOnlyQuery1.SQL.Add(',CASE WHEN position(''[u8]'' in a.uslugi_text)>0 ');
  //ZReadOnlyQuery1.SQL.Add('THEN substr(a.uslugi_text,position(''[u8]'' in a.uslugi_text)+5,(position(''|Заказной'' in a.uslugi_text))-position(''[u8]'' in a.uslugi_text)-5)::numeric ELSE 0 END as zakaz_sbor ');
  //ZReadOnlyQuery1.SQL.Add(',a.id_user, a.createdate, a.ticket_num, a.trip_date, a.trip_time, ');
  //ZReadOnlyQuery1.SQL.Add('a.mesto,a.type_full_half, a.type_norm_lgot_war, ');
  //ZReadOnlyQuery1.SQL.Add('a.type_war_unit, a.type_war_purpose, a.sum_cash, a.sum_credit, ');
  //ZReadOnlyQuery1.SQL.Add('a.lgot_id, a.lgot_percent, a.lgot_sum, a.fio, a.birthday, a.birthplace, a.tel, ');
  //ZReadOnlyQuery1.SQL.Add('a.doctype, a.doc, a.tarif_calculated ');
  //ZReadOnlyQuery1.SQL.Add(',(SELECT SUM(sum_cash) FROM av_ticket c WHERE c.type_ticket=2 AND c.type_oper=1 AND c.bagage_num<>'''' ');
  //ZReadOnlyQuery1.SQL.Add('                  AND c.refund_id_point=0 and c.ticket_num=a.ticket_num) as bags_sum ');
  //ZReadOnlyQuery1.SQL.Add(',(SELECT COUNT(*) FROM av_ticket c WHERE c.type_ticket=2 AND c.type_oper=1 AND c.bagage_num<>'''' ');
  //ZReadOnlyQuery1.SQL.Add('                  AND c.refund_id_point=0 and c.ticket_num=a.ticket_num) as bags ');
  //ZReadOnlyQuery1.SQL.Add(',(SELECT b.name FROM av_spr_point b WHERE b.del=0 AND a.id_do=b.id ORDER BY b.createdate DESC LIMIT 1 OFFSET 0) as name_kuda   ');
  //ZReadOnlyQuery1.SQL.Add(',(SELECT b.name FROM av_spr_point b WHERE b.del=0 AND a.id_ot=b.id ORDER BY b.createdate DESC LIMIT 1 OFFSET 0) as name_otkuda ');
  //If (full_mas[x,0]='3') or (full_mas[x,0]='4') then
  //  begin
  //  ZReadOnlyQuery1.SQL.Add(' FROM av_ticket_local a ');
  //  end
  //else
  //begin
  // ZReadOnlyQuery1.SQL.Add(' FROM av_ticket a ');
  //end;
  //ZReadOnlyQuery1.SQL.Add('WHERE a.id_shedule='+full_mas[x,1]);
    //- Тип рейса в массиве
            //1: из av_trip (регулярный)
            //2: из av_trip_add (заказной)
            //3: из av_ticket_local (удаленный рейс регулярный)
            //4: из av_ticket_local (удаленный рейс заказной)
  //If (full_mas[x,0]='3') or (full_mas[x,0]='4') then
  //  ZReadOnlyQuery1.SQL.Add(' AND a.trip_date='+Quotedstr(datetostr(strtodatetime(full_mas[x,11],mySettings))))
  //else
  //ZReadOnlyQuery1.SQL.Add(' AND a.trip_date='+Quotedstr(datetostr(work_date)));
  //
  //ZReadOnlyQuery1.SQL.Add(' AND a.trip_time='+QuotedStr(full_mas[x,10]));
  //ZReadOnlyQuery1.SQL.Add(' AND a.type_oper=1 ');
  //ZReadOnlyQuery1.SQL.Add('AND a.type_ticket=1 ');
  //ZReadOnlyQuery1.SQL.Add('AND a.refund_id_point=0 ');
  //ZReadOnlyQuery1.SQL.Add('AND a.unused=0 AND not a.brak ');
  //ZReadOnlyQuery1.SQL.Add('AND not(a.ticket_num='''') AND not(a.id_user=0) ORDER BY a.mesto; ');
  //showmessage(ZReadOnlyQuery1.SQL.text);//$
   try
     ZReadOnlyQuery1.open;
   except
  ...
  If ZReadOnlyQuery1.RecordCount>0 then
    ...
    for m:=0 to ZReadOnlyQuery1.RecordCount-1 do
     ...
     ///если нет номер билета, значит билет- пустышка
       ...
       tick_mas[length(tick_mas)-1,11] := StringReplace(ZReadOnlyQuery1.FieldByName('fio').AsString,'|',#32,[rfReplaceAll]);
       tick_mas[length(tick_mas)-1,12] := ZReadOnlyQuery1.FieldByName('doctype').AsString;
       tick_mas[length(tick_mas)-1,13] := ZReadOnlyQuery1.FieldByName('doc').AsString;
...
var
  n,m,k,j,bagag_count,wars_count,lgot_count,uslugi_count,tickets_count,cnt,tranzit,creditcard :integer;
...
begin
 ...
 //showmessage(ZReadOnlyQuery1.sql.Text);//$
  ...
  If length(tick_mas)>0 then
     ...
     begin
       ...
       try
        ...
        //если время продажи билета после времени создания ведомости то отвал
         ...
         begin
         showmessagealt('Ошибка преобразования времени !');
         continue;
         end;
       end;
       end;
       ...
       try
         ...
         begin
         showmessagealt('Ошибка преобразования даты !');
         continue;
         end;
       end;
       end;
   ...
   vedom_name:='vedom'+inttostr(id_user)+'.txt';
   //printer_vid:=1;

  //если опция печати на лазерный принтер
  If printer_vid=1 then
    begin
      DeleteFile(vedom_name);
      AssignFile(filetxt,vedom_name);
 ...
 //write(filetxt,StringofChar(#32,45)+under+PadC(vedom_mas[nx,2],#32,15)+plain+#13#10); //дата  перевод строки и возврат каретки
  ...
  write(filetxt,' |_____|________________________|______|_________|_________|___________|___________|______________________|_______|___________|'+#13#10);
...
//write(filetxt,' |_____|________________________|______|_________|_________|___________|_____________________|______________________|________|___________|'+#13#10);
  ...
  write(filetxt,' |_____|________________________|______|_________|_____________________|___________|______________________|_______|___________|'+#13#10);
     end;
    end
  else
  begin
  //              | 5   |         24             |  6   |   9     |            21       |   7   |    11     |
  write(filetxt,' ___________________________________________________________________________________________ '+#13#10);
  write(filetxt,' |     |                        |      |         |                     |Квитанция на провоз|'+#13#10);
...
//
  ...
  write(filetxt,' |'+bold+PadL(tick_mas[n,0],#32,5)+'|'); //номер места
  ...
  begin
   ...
   If fldoc OR flfio  then
     ...
     write(filetxt,PadC(utf8copy(tick_mas[n,13],1,10),#32,11)+'|');//тип документа, номер
  ...
  If (fldoc OR flfio) then
   ...
   If ((utf8length(tick_mas[n,13])>10) or (utf8length(tick_mas[n,11])>22)) then
     begin
     cnt:=cnt+1;
 ...
 //ЕСЛИ ПЕЧАТЬ СПИСКА ПАССАЖИРОВ
  If spass=1 then
   //                | 5   |         24             |  6   |   9     |    9    |     11    |     11    |        22            |    7   |    11     |
     write(filetxt,' |_____|________________________|______|_________|_________|___________|___________|______________________|________|___________|'+#13#10)
     else
  //                 | 5   |         24             |  6   |   9     |         21          |     11    |        22            |    7   |    11     |
     write(filetxt,' |_____|________________________|______|_________|_____________________|___________|______________________|________|___________|'+#13#10)
  else
    //               | 5   |         24             |  6   |   9     |         21          |   7   |    11     |
     write(filetxt,' |_____|________________________|______|_________|_____________________|_______|___________|'+#13#10);
  ...
  write(filetxt,' |_____|________________________|______|_________|_________|___________|___________|______________________|_______|___________|'+#13#10);
     ...
     begin
  //              | 5   |         24             |  6   |   9     |         21          |     11    |        22            |   7   |    11     |
  write(filetxt,' ______________________________________________________________________________________________________________________________ '+#13#10);
  write(filetxt,' |     |                        |      |         |                     |           |                      |Квитанция на провоз|'+#13#10);
  write(filetxt,' |Номер|          Номер         | Вид* |Стоимость|        Пункт        | Документ  |        Ф. И. О.      |багажа/ручной клади|'+#13#10);
  write(filetxt,' |места|         билета         |билета|         |     назначения      |           |                      | кол-во|   сумма   |'+#13#10);
  write(filetxt,' |_____|________________________|______|_________|_____________________|___________|______________________|_______|___________|'+#13#10);
...
//
  ...
  begin
    //если это билет не этой ведомости, то пропускаем
    If tick_mas[n,4]<>'1' then continue;
  write(filetxt,' |'+bold+PadC(tick_mas[n,0],#32,5)+'|'); //номер места
  write(filetxt,PadR(tick_mas[n,1],#32,24)+norm+'|');    //номер билета
  write(filetxt,PadC(utf8copy(tick_mas[n,16],1,6),#32,6)+'|');     //вид билета
  write(filetxt,PadC(tick_mas[n,10],#32,9)+'|');     //стоимость
    //ЕСЛИ ПЕЧАТЬ СПИСКА ПАССАЖИРОВ
 ...
 closefile(filetxt);
  ...
  {$IFDEF WINDOWS}
     //создаем html
     // Начало HTML
 if StartHTML(ExtractFilePath(Application.ExeName)+'vedom'+inttostr(id_user)+'.html')=false then exit;
 // Таблица - Начало
...
//CellsTableHTML('',1,1,1,'000000',0,0,'');
 ...
 startbrowser(ExtractFilePath(Application.ExeName)+'vedom'+inttostr(id_user)+'.html');
  {$ENDIF}
...
//The printer language ESC/P was originally developed by Epson for use with their early dot-matrix printers.
  ...
  write(filetxt,'Время отправления '+#27#69+PadC(vedom_mas[nx,10],#32,10)+#27#70+' Время отправления фактически '
   ...
   for n:=0 to length(full_mas)-2 do
     begin
      ...
      tn1:=strtoint(copy(t1,1,2)+copy(t1,4,2));
        ...
        on exception: EConvertError do exit; //showmessage('1  '+inttostr(n)+'|'+full_mas[n,1]+'|'+full_mas[n,10]+' | '+full_mas[n,12]);
      end;
       for m:=n to length(full_mas)-1 do
        ...
        try
           ...
           on exception: EConvertError do exit; //showmessage('2  '+inttostr(m)+'|'+inttostr(m)+'| '+full_mas[m,1]+'|'+full_mas[n,10]+' | '+full_mas[m,12]);
        end;
      ...
      If (key=118) then
        begin
         ...
         If not(Connect2(form1.Zconnection2, 1)) then
          begin
             showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k5-');
             exit;
          end;
          try
...
// --------------------------------------------------------------------------
 ...
 for n:=ot to pokuda do
  ...
  If not(Connect2(form1.Zconnection2, 1)) then
   begin
    showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k7-');
    exit;
   end;
   end;
      ...
      if (aCol=0) then
       ...
       //6 - ЗАКРЫТ ПРИНУДИТЕЛЬНО
          begin
            ...
            //6 - ЗАКРЫТ ПРИНУДИТЕЛЬНО
            if (form1.StringGrid1.Cells[aCol,Arow]='6') then
                begin
                  font.color:=clRed;
...
begin
  //t1 := time();
   ...
   If not(Connect2(form1.Zconnection1, 2)) then
     begin
      showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k8-');
      //closeAll();
      halt;
      exit;
     end;
  ...
  //showmessage(ZReadOnlyQuery1.SQL.text);//$
   ...
   If not LPTonly and FileExistsUTF8('/dev/usb/lp0') then
   begin
    //try
 ...
 //ПРОВЕРКА ЗАГРУЖЕННЫХ ОПЦИЙ
   ...
   un_critical_time:=strtotime(inttostr(unused_critical_time div 60)+':'+inttostr(unused_critical_time mod 60));
 ...
 if flag1=0 then
  begin
   ...
   //form1.Button3.Click;
   If ((id_user=1) or (id_user=2)) and (days_before<60) then days_before:=60;//& кол-во прошедших дней
end;
...
begin
  ...
  If not(Connect2(form1.Zconnection2, 2)) then
   begin
    showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...'+#13+'-k9-');
    exit;
   end;
   active_check:=true; //флаг выполнения обновления
...
begin
  ...
  try
    company_inn := Ini.ReadString('global','inn', '22013');
    company_name := Ini.ReadString('global','company', '07701000');
    If Ini.ReadString('global','printer_interface', 'usb')='LPT' then
      LPTonly:=true else LPTonly:=false;

    webtick1:=Ini.ReadString('global','webticket1','Билет сформирован посредством автоматизированной системы');
    webtick2:=Ini.ReadString('global','webticket2','"ПЛАТФОРМА-АВ" (ОАО "АВТОВОКЗАЛ", ИНН 2634026147)');
    webtick3:=Ini.ReadString('global','webticket3','Билет заказан через Интернет-сайт www.stavbilet26.ru (ООО "Стававто", ИНН 263465234)');
    webtick4:=Ini.ReadString('global','webticket4','телефон службы поддержки, +7(8652)23-47-20; e-mail:support@stavbilet26.ru');
  finally
    Ini.Free;
  end;
end;
