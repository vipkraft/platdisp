...
type
  ...
  TForm2 = class(TForm)
   ...
   procedure insert_oper_virt();//запись удаленной операции локально
   procedure bron_correct();//проверка корректности брони после операции
   function ats_shtraf_check():boolean;//проверка необходимости печати АКТ незаход
   function inetdeny():boolean;//проверка на запрет продажи через сайт
   function fiomust():boolean;//проверка на необходимость внесения ФИО на заказных рейсах
   procedure inetdeny_ins();//проверка на запрет продажи через сайт
   procedure fiomust_ins();//проверка на необходимость внесения ФИО на заказных рейсах
   procedure check_seat_off();//проверка на потерянные места
   procedure first_find();
  private
...
var
  ...
  notfindflag,flseek,checkstate,dontcheck:boolean;
  famildlina:byte;
  ggg:integer;
  tekedit:integer;
  nfirstmest:integer;
...
procedure TForm2.first_find();
begin
//********  2020 10 01 ********** автоматическое внесение данных водителя на форму *
If (operation>1) or (form2.edit2.text<>'') then exit;
...
If not(Connect2(form2.Zconnection1, 1)) then
 ...
 form2.ZReadOnlyQuery1.SQL.Clear;
   form2.ZReadOnlyQuery1.SQL.Add('SELECT * from find_voditel('+full_mas[idx,1]+','+Quotedstr('')+','+Quotedstr('')+');');
...
If form2.ZReadOnlyQuery1.RecordCount=1 then
   begin
 //заполняем едиты фамилий
    decode_personal(2,form2.ZReadOnlyQuery1.Fields[0].AsString);
    //заполняем остальные поля
    decode_personal(13,form2.ZReadOnlyQuery1.Fields[1].AsString);
    flchange:=true;
   end;
 form2.ZReadOnlyQuery1.Close;
 form2.Zconnection1.disconnect;
...
procedure TForm2.check_seat_off();//проверка на потерянные места
var
  n:integer;
begin
 mesta:='';
  ...
  form2.ZReadOnlyQuery1.SQL.Clear;
  form2.ZReadOnlyQuery1.SQL.Add('select mesto from av_ticket where trip_date='+quotedstr(tripdate)+' and trip_time='+quotedstr(full_mas[idx,10]));
  form2.ZReadOnlyQuery1.SQL.Add(' AND id_shedule='+full_mas[idx,1]+' AND mesto>'+ats_mest+' AND id_user>0 and type_oper=1 and not brak order by mesto desc;');
  //showmessage(form2.ZReadOnlyQuery1.SQL.Text);//$
  try
      form2.ZReadOnlyQuery1.open;
  except
  ...
  IF form2.ZReadOnlyQuery1.RecordCount=0 then
    begin
     ...
     for n:=1 to form2.ZReadOnlyQuery1.RecordCount do
       begin
         mesta:=mesta+trim(form2.ZReadOnlyQuery1.FieldByName('mesto').asString)+',';
         form2.ZReadOnlyQuery1.Next;
       end;

  form2.ZReadOnlyQuery1.Close;
  form2.Zconnection1.disconnect;
  showmessagealt('В Н И М А Н И Е !!!'+#13+'Прозведите операцию бронирования посадочных '+mesta+' мест,'+#13
      +'в связи с имеющейся продажей билетов на '+inttostr(n)+' места.'+#13);
 end;


//внести признак необходимости внесения фио пассажира на заказном рейсе
procedure TForm2.fiomust_ins();
begin
        If not(Connect2(form2.Zconnection1, 1)) then
    begin
...
try
 If not form2.ZConnection1.InTransaction then
   begin
    form2.ZConnection1.StartTransaction;
   end
 else
   begin
...
//запись операции
If not form2.CheckBox3.Checked then
  begin
form2.ZReadOnlyQuery1.SQL.add('Update av_disp_inet_deny SET del=3, id_user='+inttostr(id_user)+', createdate=now() WHERE del=2 ');//AND id_point_oper='+ConnectINI[14]);
form2.ZReadOnlyQuery1.SQL.add(' AND trip_date='+Quotedstr(tripdate));
form2.ZReadOnlyQuery1.SQL.add(' AND id_shedule='+full_mas[idx,1]+' AND trip_time='+QuotedStr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+';');
   end
else
begin
  form2.ZReadOnlyQuery1.SQL.add('INSERT INTO av_disp_inet_deny(id_shedule,trip_date,trip_time,createdate,id_user,del,createdate_first,id_user_first) VALUES (');
  form2.ZReadOnlyQuery1.SQL.add(full_mas[idx,1]+','+Quotedstr(tripdate)+','+QuotedStr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+',now(),'+inttostr(id_user)+',2,now(),'+inttostr(id_user));
  form2.ZReadOnlyQuery1.SQL.add(');');
end;
  //showmessage(form2.ZReadOnlyQuery1.SQL.Text);//$
  form2.ZReadOnlyQuery1.ExecSql;
  form2.ZConnection1.Commit;
except
   If form2.ZConnection1.InTransaction then form2.ZConnection1.Rollback;
   form2.ZConnection1.disconnect;
   showmessagealt('Данные не записаны !'+#13+'Не удается завершить транзакцию !!!'
   //+#13+'Запрос SQL1: '+form2.ZReadOnlyQuery1.SQL.Text
   +#13+'Запрос SQL1: '+form2.ZReadOnlyQuery1.SQL.Text);
   exit;
end;
form2.ZReadOnlyQuery1.Close;
...
If form2.CheckBox2.Checked then
  begin
form2.ZReadOnlyQuery1.SQL.add('Update av_disp_inet_deny SET del=1, id_user='+inttostr(id_user)+', createdate=now() WHERE del=0 ');//AND id_point_oper='+ConnectINI[14]);
form2.ZReadOnlyQuery1.SQL.add(' AND trip_date='+Quotedstr(tripdate));
form2.ZReadOnlyQuery1.SQL.add(' AND id_shedule='+full_mas[idx,1]+' AND trip_time='+QuotedStr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+';');
   end
else
begin
  form2.ZReadOnlyQuery1.SQL.add('INSERT INTO av_disp_inet_deny(id_shedule,trip_date,trip_time,createdate,id_user,del,createdate_first,id_user_first) VALUES (');
  form2.ZReadOnlyQuery1.SQL.add(full_mas[idx,1]+','+Quotedstr(tripdate)+','+QuotedStr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+',now(),'+inttostr(id_user)+',0,now(),'+inttostr(id_user));
  form2.ZReadOnlyQuery1.SQL.add(');');
end;
  //showmessage(form2.ZReadOnlyQuery1.SQL.Text);//$
  form2.ZReadOnlyQuery1.ExecSql;
...
//проверка на запрос необходимости внесения ФИО для заказных рейсов
function TForm2.fiomust():boolean;
 var
  k:integer;
 stmp,seat,fromseat:string;
begin
    result:=false;
    If not(Connect2(form2.Zconnection1, 1)) then
    begin
     ...
     form2.close;
      ...
      form2.ZReadOnlyQuery2.sql.Clear;
      form2.ZReadOnlyQuery2.sql.add('Select 1 FROM av_disp_inet_deny where del=2 and id_shedule='+full_mas[idx,1]);
      form2.ZReadOnlyQuery2.sql.add(' and trip_date='+quotedstr(tripdate)+' and trip_time='+quotedstr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+' order by createdate limit 1;');
      //showmessage(form2.ZReadOnlyQuery2.sql.Text);//$
         try
           form2.ZReadOnlyQuery2.open;
         except
         showmessagealt('ОШИБКА ЗАПРОСА !'+#13+'Команда: '+form2.ZReadOnlyQuery2.SQL.Text);
         form2.ZReadOnlyQuery2.Close;
         Zconnection1.disconnect;
         exit;
         end;
      if form2.ZReadOnlyQuery2.RecordCount>0 then
          result:=true;
...
function TForm2.inetdeny():boolean;//проверка на запрет продажи через сайт
 var
  k:integer;
 stmp,seat,fromseat:string;
begin
    ...
    begin
     ...
     form2.close;
      ...
      form2.ZReadOnlyQuery2.sql.Clear;
      form2.ZReadOnlyQuery2.sql.add('Select 1 FROM av_disp_inet_deny where del=0 and id_shedule='+full_mas[idx,1]);
      form2.ZReadOnlyQuery2.sql.add(' and trip_date='+quotedstr(tripdate)+' and trip_time='+quotedstr(formatdatetime('hh:nn',form2.DateTimePicker1.Time))+' order by createdate limit 1;');
      //showmessage(form2.ZReadOnlyQuery2.sql.Text);//$
         try
           form2.ZReadOnlyQuery2.open;
...
function TForm2.ats_shtraf_check():boolean;//проверка необходимости печати АКТ незаход
var
  k:integer;
 stmp,seat,fromseat:string;
begin
    ...
    //выясняем величину штрафа за незаход
        form2.ZReadOnlyQuery2.sql.Clear;
        form2.ZReadOnlyQuery2.sql.add('Select id_kontr FROM av_spr_kontr_dog where del=0 and shtr8>0 and id_kontr='+kontr_id+' order by createdate limit 1;');
...
procedure TForm2.bron_correct();//проверка корректности брони после операции
var
  k:integer;
 stmp,seat,fromseat:string;
begin
   //если формирующийся - выход
    If full_mas[idx,9]='1' then exit;
   try
   If nfirstmest <=strtoint(ats_mest) then exit;
   except
      on exception: EConvertError do
        begin
            showmessagealt('Ошибка конвертации числа мест !'+#13+'--err36');
          exit;
          end;
   end;
    ...
    //выясняем максимальное забронированное место
        form2.ZReadOnlyQuery2.sql.Clear;
        form2.ZReadOnlyQuery2.sql.add('select disp_bron(''dbr'','+quotedstr(tripdate)+','+full_mas[idx,1]+','+quotedstr(full_mas[idx,10])+','+quotedstr('-3-')+');');
        form2.ZReadOnlyQuery2.sql.add('FETCH ALL IN dbr;');
        //showmessage(form2.ZReadOnlyQuery2.sql.Text);//$
         try
           form2.ZReadOnlyQuery2.open;
         except
      ...
      if form2.ZReadOnlyQuery2.RecordCount>0 then
        begin
          stmp:=form2.ZReadOnlyQuery2.FieldByName('mesta').asString;
        end;
      seat:='';
      for k:=utf8length(stmp) downto 1 do
      begin
       If utf8copy(stmp,k,1)=',' then break;
       seat:=utf8copy(stmp,k,1) + seat;
      end;
      //showmessage(seat);
      fromseat:='';
       try
        If seat<>'' then
         If strtoint(seat)>strtoint(ats_mest) then fromseat:=seat;
       except
       end;

        //выясняем максимальное открытое диспетчером место на транзите
        //form2.ZReadOnlyQuery2.close;
        form2.ZReadOnlyQuery2.sql.Clear;
        form2.ZReadOnlyQuery2.sql.add('select disp_bron(''dd'','+quotedstr(tripdate)+','+full_mas[idx,1]+','+quotedstr(full_mas[idx,10])+','+quotedstr('-0-')+');');
        form2.ZReadOnlyQuery2.sql.add('FETCH ALL IN dd;');
        //showmessage(form2.ZReadOnlyQuery2.sql.Text);//$
         try
           form2.ZReadOnlyQuery2.open;
         except
      ...
      seat:='';
      for k:=1 to utf8length(stmp) do
      begin
       If utf8copy(stmp,k,1)=',' then
        begin
          If seat='' then continue;
           try
             If fromseat<>'' then
              begin
               If strtoint(seat)>strtoint(fromseat) then
                 begin
                 fromseat:=seat;
                 break;
                 end;
             ...
             begin
               If strtoint(seat)>strtoint(ats_mest) then
                 begin
                 fromseat:=seat;
                 break;
                 end;
       ...
       seat:=seat + utf8copy(stmp,k,1);
      end;
      //showmessage(seat);

     If fromseat='' then exit;
 //========================= запрашиваем бронь и сразу ее меняем
 form2.ZReadOnlyQuery2.sql.Clear;
 form2.ZReadOnlyQuery2.sql.add('SELECT id_shedule, id_user, createdate, del, time_trip, ot_id_point,');
 form2.ZReadOnlyQuery2.sql.add('do_id_point, id_ats, id_kontr, substr(bron,1,position(''|''||'+fromseat+' in bron)-1) as bron, ot_point_order, do_point_order,');
 form2.ZReadOnlyQuery2.sql.add('id_point, date_trip from av_disp_bron a WHERE a.del=0 AND a.date_trip='+quotedstr(tripdate));
 form2.ZReadOnlyQuery2.sql.add(' AND a.id_shedule='+full_mas[idx,1]+' AND a.time_trip='+quotedstr(full_mas[idx,10])+' AND a.ot_point_order='+full_mas[idx,4]+';');
...
try
  ...
  //запись операции
  form2.ZReadOnlyQuery1.sql.Clear;
  form2.ZReadOnlyQuery1.SQL.add('Update av_disp_bron SET del=1 WHERE del=0 ');//AND id_point_oper='+ConnectINI[14]);
  form2.ZReadOnlyQuery1.SQL.add(' AND date_trip='+Quotedstr(tripdate));
  form2.ZReadOnlyQuery1.SQL.add(' AND id_shedule='+full_mas[idx,1]+' AND time_trip='+QuotedStr(up_time));
  form2.ZReadOnlyQuery1.SQL.add(' AND ot_point_order='+up_order+';');
  form2.ZReadOnlyQuery1.SQL.add('INSERT INTO av_disp_bron(');
         for k:=0 to form2.ZReadOnlyQuery2.FieldCount-2 do
         begin
             form2.ZReadOnlyQuery1.SQL.add((trim(form2.ZReadOnlyQuery2.Fields[k].FieldName))+',');
         end;
             form2.ZReadOnlyQuery1.SQL.add((trim(form2.ZReadOnlyQuery2.Fields[form2.ZReadOnlyQuery2.FieldCount-1].FieldName)));
     form2.ZReadOnlyQuery1.SQL.add(') VALUES (');
         for k:=0 to form2.ZReadOnlyQuery2.FieldCount-2 do
          begin
             form2.ZReadOnlyQuery1.SQL.add(
                    IFTHEN(form2.ZReadOnlyQuery2.Fields[k].AsString='',
                       ifthen(form2.ZReadOnlyQuery2.Fields[k].IsNull,'null',quotedstr(''))
                      ,quotedstr(form2.ZReadOnlyQuery2.Fields[k].AsString))+',');
          end;
             form2.ZReadOnlyQuery1.SQL.add(quotedstr(form2.ZReadOnlyQuery2.Fields[form2.ZReadOnlyQuery2.FieldCount-1].text));
    form2.ZReadOnlyQuery1.SQL.add(');');
    //showmessagealt(form2.ZReadOnlyQuery1.SQL.Text);//$
    form2.ZReadOnlyQuery1.ExecSql;
    form2.ZConnection1.Commit;
except
     If form2.ZConnection1.InTransaction then form2.ZConnection1.Rollback;
...
//открытие\закрытие элементов для редактирования
...
begin
  ...
  If mode=0 then
   begin
        form2.ZReadOnlyQuery1.SQL.Add('SELECT * from find_voditel('+ished+','+Quotedstr(tdrv)+','+Quotedstr(strn)+');');
   // If isfio then
   //  begin
   ...
   // form2.ZReadOnlyQuery1.SQL.Add(' id_shedule='+ished+' and createdate>(now() - interval ''30 days'') and driver2 like '+Quotedstr(tdrv+'%'));
   // form2.ZReadOnlyQuery1.SQL.Add(' ) ) c ');
   // form2.ZReadOnlyQuery1.SQL.Add(' order by char_length(doc) desc, dd limit 15; ');
   //  end;
   // If not isfio then
   // begin
   ...
   // form2.ZReadOnlyQuery1.SQL.Add(' id_shedule='+ished+' and createdate>(now() - interval ''30 days'') and driver2 like '+Quotedstr(tdrv+'%'));
   // form2.ZReadOnlyQuery1.SQL.Add(' ) ) c ');
   // form2.ZReadOnlyQuery1.SQL.Add(' order by dd limit 15; ');
   // end;
   end;
...
//ищем по документу
  ...
  // form2.ZReadOnlyQuery1.SQL.Add(' createdate>(now() - interval ''30 days'') and driver3 like '+Quotedstr('%'+tdrv+'%'));
  // form2.ZReadOnlyQuery1.SQL.Add(') union all ');
  ...
  // form2.ZReadOnlyQuery1.SQL.Add(' createdate>(now() - interval ''30 days'') and driver4 like '+Quotedstr('%'+tdrv+'%'));
  // form2.ZReadOnlyQuery1.SQL.Add(' ) ) c ');
  // form2.ZReadOnlyQuery1.SQL.Add(' order by char_length(doc) desc limit 15;');
  end;
 ...
 result:= form2.ZReadOnlyQuery1.RecordCount;

If result>0 then begin
for n:=1 to result do
  begin
    inc(k);
...
begin
  //form2.Label38.Caption:=inttostr(famildlina);//$
   ...
   If TEdit(Sender).SelLength>0 then
    begin
      //writelog('Form2.Edit2KeyUp TEdit(Sender).SelText:  '+TEdit(Sender).SelText);//$
      exit;
    end;

   //если уже происходит поиск водителя - отвал
   IF flseek then exit;

   //если удаление символов - отвал
   If notfindflag then exit;

   //writelog('Edit2KeyUp  TEdit(Sender).Text:  '+TEdit(Sender).Text);//$
   ...
   //writelog('TEdit(Sender).Text:  '+TEdit(Sender).Text);//$

     If nfnd>1 then begin
   ...
   begin
    ...
    form2.stringgrid1.ColWidths[1]:=form2.stringgrid1.Width - 5;
    form2.stringgrid1.ColWidths[0]:=0
      end;
   end;
   //Если не нашли, выход
   If nfnd=0 then exit;
   //Если 1 совпадение, выход
   If nfnd=1 then
    begin
     //writelog('Form2.Edit2KeyUp dolen:  '+inttostr(dolen));//$
    ...
    doctype:=0;
     If trim(Edit13.Caption)<>'' then
      begin
       ...
       16: doctype:=18;
       17: doctype:=99;
       else doctype:=form2.ComboBox1.ItemIndex;
       end;
      doc1:=StringReplace(trim(Edit13.Caption),#32,'',[rfReplaceAll])+'|'+inttostr(doctype)+'|'+
    ...
    doctype:=0;
     If trim(Edit15.Caption)<>'' then
      begin
       ...
       16: doctype:=18;
       17: doctype:=99;
       else doctype:=form2.ComboBox3.ItemIndex;
       end;
      doc2:=StringReplace(trim(Edit15.Caption),#32,'',[rfReplaceAll])+'|'+inttostr(doctype)+'|'+
    ...
    doctype:=0;
     If trim(Edit17.Caption)<>'' then
      begin
       ...
       16: doctype:=18;
       17: doctype:=99;
       else doctype:=form2.ComboBox5.ItemIndex;
       end;
      doc3:=StringReplace(trim(Edit17.Caption),#32,'',[rfReplaceAll])+'|'+inttostr(doctype)+'|'+
...
begin
  ...
  while utf8pos('|',str)>0 do
   begin
     ...
     //Новые типы документов 20180501
     If trim(tmp)='18' then tmp:=inttostr(Combobox1.Items.Count-2);
     If trim(tmp)='99' then tmp:=inttostr(Combobox1.Items.Count-1);
     If drv=13 then
     begin
     try
...
const
 ...
 var
   ...
   //prn,filev: String;
begin
   //prn := '/dev/lp0';//заремарино 20180508
   //AssignFile(buf_lpt,prn);заремарино 20180508
...
begin
  ...
  //ищем по всему массиву
      ...
      begin
       //id_shedule должен совпадать
        ...
        //если все реквизиты совпадают (один отрезок)
                      If
   ...
   //подключаемся к базе и проверяем время между пунктами (движения + стоянки)
   //если меньше 10 часов, то добавить отрезок рейса в связанный
  //If not(Connect2(form2.Zconnection1, 1)) then
  //  begin
  //   showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...');
  //   break;
  //  end;
  //    form2.ZReadOnlyQuery2.sql.Clear;
  //    //////////////////////////////////////////////////////////////// id_shedule           order_otkuda        order_otkuda
  //    form2.ZReadOnlyQuery2.sql.add('Select disp_vputi_time_valid('+full_mas[idx,1]+','+full_mas[idx,46]+','+full_mas[n,46]+');');
  //    //showmessage(form2.ZReadOnlyQuery2.sql.Text);//$
  //       try
  //         form2.ZReadOnlyQuery2.open;
  //       except
  ...
  //    if form2.ZReadOnlyQuery2.RecordCount>0 then
  //      begin
  //      //если между пунктами отрезков больше 10 часов, то пропуск
  //        If not form2.ZReadOnlyQuery2.Fields[0].AsBoolean then
  //          begin
  //             form2.ZReadOnlyQuery2.Close;
  //             form2.ZConnection1.disconnect;
  //             continue;
                   ...
                   If (full_mas[n,16]='1') then
                     begin
                       try
                         ...
                         IF strtotime(up_time)<=strtotime(full_mas[n,10]) then
                           begin
                             //добавить рейс в массив
                          setlength(megamas,length(megamas)+1);
                          megamas[length(megamas)-1]:=n;
                  ...
                  //если рейс прибытия
                    If (full_mas[n,16]='2') then
                     begin
                       try
                           //если время отправления/прибытия больше, чем в операции
                         IF strtotime(up_time)<=strtotime(full_mas[n,12]) then
                           begin
                             //добавить рейс в массив
                          setlength(megamas,length(megamas)+1);
                          megamas[length(megamas)-1]:=n;
...
begin
 ...
 ////Вычисляем номер ведомости
  ...
  ZQ1.SQL.Add(full_mas[nx,6]+','+mode+','+ConnectINI[14]+','+inttostr(id_user));
  //ZQ1.SQL.Add(',now()+''2 seconds'',0,');//заремарил 21,11,2016 иногда не садятся билеты в дообилечивание
  ZQ1.SQL.Add(',now(),0,');
...
//запись операции на виртуальном сервере себе в локальную базу
procedure TForm2.insert_oper_virt();
var
  k:integer;
begin
// Подключаемся к удаленному серверу
   If not(Connect2(form2.ZConnection2, 1)) then
     begin
      showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...');
      exit;
     end;
  form2.ZReadOnlyQuery2.Connection:=form2.ZConnection2;
  form2.ZReadOnlyQuery2.sql.Clear;
  form2.ZReadOnlyQuery2.SQL.add('Select * from av_disp_oper WHERE del=0 ');//AND id_point_oper='+ConnectINI[14]);
  form2.ZReadOnlyQuery2.SQL.add(' AND trip_date='+Quotedstr(tripdate)+' AND trip_type='+full_mas[idx,16]);
  form2.ZReadOnlyQuery2.SQL.add(' AND id_shedule='+full_mas[idx,1]+' AND trip_time='+QuotedStr(up_time)+' AND trip_id_point='+up_point);
  form2.ZReadOnlyQuery2.SQL.add(' AND point_order='+up_order+' ORDER BY createdate limit 1;');
  //showmessage(form2.ZReadOnlyQuery2.SQL.Text);//$
     try
      form2.ZReadOnlyQuery2.Open;
      except
  ...
  If form2.ZReadOnlyQuery2.RecordCount=0 then
   begin
     form2.ZReadOnlyQuery2.Close;
     form2.Zconnection2.disconnect;
     showmessagealt('НЕ найден по реквизитам рейса:'+#13+' trip_date='+Quotedstr(tripdate)+' AND trip_type='+full_mas[idx,16]+#13+
                 'AND id_shedule='+full_mas[idx,1]+' AND trip_time='+QuotedStr(up_time)+#13+' AND trip_id_point='+up_point+' AND point_order='+up_order);
     exit;
   end;

  // Подключаемся к локальному серверу
   If not(Connect2(form2.ZConnection1,2)) then
     begin
      showmessagealt('Соединение с сервером базы данных отсутствует !'+#13+'Проверьте сетевое соединение и опции файла настроек системы...');
      exit;
     end;
...
try
  ...
  //запись операции
  form2.ZReadOnlyQuery1.sql.Clear;
  form2.ZReadOnlyQuery1.SQL.add('Update av_disp_oper SET del=1 WHERE del=0 ');//AND id_point_oper='+ConnectINI[14]);
  form2.ZReadOnlyQuery1.SQL.add(' AND trip_date='+Quotedstr(tripdate)+' AND trip_type='+full_mas[idx,16]);
  form2.ZReadOnlyQuery1.SQL.add(' AND id_shedule='+full_mas[idx,1]+' AND trip_time='+QuotedStr(up_time)+' AND trip_id_point='+up_point);
  form2.ZReadOnlyQuery1.SQL.add(' AND point_order='+up_order+';');
   form2.ZReadOnlyQuery1.SQL.add('INSERT INTO av_disp_oper(');
         for k:=0 to form2.ZReadOnlyQuery2.FieldCount-2 do
         begin
             form2.ZReadOnlyQuery1.SQL.add((trim(form2.ZReadOnlyQuery2.Fields[k].FieldName))+',');
         end;
     ...
     form2.ZReadOnlyQuery1.SQL.add(') VALUES (');
         for k:=0 to form2.ZReadOnlyQuery2.FieldCount-2 do
          begin
             form2.ZReadOnlyQuery1.SQL.add(
                    IFTHEN(form2.ZReadOnlyQuery2.Fields[k].Text='',
                       ifthen(form2.ZReadOnlyQuery2.Fields[k].IsNull,'null',quotedstr(''))
                      ,quotedstr(form2.ZReadOnlyQuery2.Fields[k].text))+',');
          end;
    form2.ZReadOnlyQuery1.SQL.add(quotedstr(form2.ZReadOnlyQuery2.Fields[form2.ZReadOnlyQuery2.FieldCount-1].text));
    form2.ZReadOnlyQuery1.SQL.add(');');
  ...
  //If (lg<>4) and (lg<>1) and (lg<>5) and (lg<>7) then form2.ZReadOnlyQuery1.Close;
  form2.ZReadOnlyQuery1.Close;
  form2.ZReadOnlyQuery2.Close;
  form2.ZConnection2.disconnect;
...
begin
 ...
 try
  ...
  begin
     //пишем номер операции (id_oper)
    If (mode=1) and (oper=1) then
    //просто сохранить реквизиты
      ZReadOnlyQuery2.SQL.Add('99,')
    else
    //выполняем операцию
       ZReadOnlyQuery2.SQL.Add(inttostr(oper)+',');

    //пишем платформу и тип рейса (zakaz)
    ZReadOnlyQuery2.SQL.Add(full_mas[idx,2]+','+full_mas[idx,0]);


            //пишем новый статус рейса (trip_flag)
             If (mode=1) then ZReadOnlyQuery2.SQL.Add(','+full_mas[idx,28]+','); //не изменился
             If (mode=2) then
              begin
               If oper=1 then  ZReadOnlyQuery2.SQL.Add(',4,'); //отправка
               If oper=2 then  ZReadOnlyQuery2.SQL.Add(',2,'); //прибытие
               end;
        //остальные реквизиты
        ZReadOnlyQuery2.SQL.Add(kontr_id+','+QuotedStr(kontr_name)+','+ats_id+',');
        ZReadOnlyQuery2.SQL.Add(quotedstr(ats_name+' ГН:'+ats_gos)+','+ats_mest+',');
 ...
 If (oper=5) or (oper=7) then
   begin
    //пишем новый статус рейса (trip_flag)
   //ZReadOnlyQuery2.SQL.Add(','+full_mas[idx,28]+','); //не изменился
   ZReadOnlyQuery2.SQL.Add(kontr_id+','+QuotedStr(kontr_name)+','+ats_id+',');
   ZReadOnlyQuery2.SQL.Add(quotedstr(ats_name+' ГН:'+ats_gos)+','+ats_mest+',');
   ZReadOnlyQuery2.SQL.Add(ats_type+','+QuotedStr(Edit6.text)+',');
   ZReadOnlyQuery2.SQL.Add(QuotedStr(drv1)+','+QuotedStr(drv2)+',');
     ...
     begin
       ...
       If copy(form1.ZReadOnlyQuery1.FieldByName('sale').asString,1,1)='1' then
        ...
        If (oper=1) or (oper=2) THEN
            begin
             //пишем номер операции (id_oper)
                If (mode=1) and (oper=1) then
                //просто сохранить реквизиты
                 form1.ZReadOnlyQuery3.SQL.Add('99,')
                else
               //выполняем операцию
                form1.ZReadOnlyQuery3.SQL.Add(inttostr(oper)+',');
           //пишем платформу и тип рейса (zakaz)
           form1.ZReadOnlyQuery3.SQL.Add(full_mas[idx,2]+','+full_mas[idx,0]);

            //пишем новый статус рейса (trip_flag)
             If (mode=1) then form1.ZReadOnlyQuery3.SQL.Add(',0,'); //не изменился
             If (mode=2) then
              begin
               If oper=1 then  form1.ZReadOnlyQuery3.SQL.Add(',4,'); //отправка
               If oper=2 then  form1.ZReadOnlyQuery3.SQL.Add(',2,'); //прибытие
               end;
             //остальные реквизиты
        ...
        If (oper=5) or (oper=7) then
          begin
           //пишем новый статус рейса (trip_flag)
          //form1.ZReadOnlyQuery3.SQL.Add(',0,'); //не изменился
          form1.ZReadOnlyQuery3.SQL.Add(kontr_id+','+QuotedStr(kontr_name)+','+ats_id+',');
          form1.ZReadOnlyQuery3.SQL.Add(quotedstr(ats_name+' ГН:'+ats_gos)+','+ats_mest+',');
          form1.ZReadOnlyQuery3.SQL.Add(ats_type+','+QuotedStr(Edit6.text)+',');
          form1.ZReadOnlyQuery3.SQL.Add(QuotedStr(drv1)+','+QuotedStr(drv2)+',');
...
try
   ...
   ZReadOnlyQuery1.SQL.Add(','+QuotedStr(doc2));//driver4
...
//remark 20210611   ZReadOnlyQuery1.SQL.add(' AND id_trip_do='+full_mas[perenos_biletov,6]+' AND order_trip_do='+full_mas[perenos_biletov,7]);
  ...
  If form2.CheckBox3.Checked then
     If not form2.fiomust()
       then form2.fiomust_ins();



 end;
 If add_trip=false then showmessagealt('БИЛЕТЫ УСПЕШНО ПЕРЕНЕСЕНЫ НА ЗАКАЗНОЙ РЕЙС !');
 active_check:=false;
 flchange:=false;
 form2.Close;
end;
 ...
 //если активна фаза сохранения, то пропускаем
  ...
  // F1
   ...
   if (Key=121) then
       begin
         key:=0;
         If (operation=5) or (operation=7) then exit;
         form2.getz_date();
...
// Вниз на АТC
  if (Key=40) and (form2.edit7.Focused=true) then
   ...
   if (Key=114) and (form2.edit1.Focused=true) then
       begin
          Key:=0;
          //НЕ ОТКРЫВАТЬ ЕСЛИ СМЕНА АТС ИЛИ ОТПРАВЛЕНИЕ
          If operation=7 then exit;
          If operation=1 then exit;
   ...
   if (Key=27) and (form2.stringgrid1.Visible=true) then
       begin
         ...
         If tekedit=2 then
           begin
           //writelog('Key=27:  '+form2.Edit2.Text);//$
   ...
   if (form2.Edit8.Visible=false) and (form2.stringgrid1.Visible=false) and (form2.Edit7.focused) then
    ...
    If ((key=8) or (key>45)) and form2.stringgrid1.Visible and form2.stringgrid1.Focused
     ...
     begin
          If tekedit=2 then
           begin
           //writelog('Key=8 or (key>45:  '+form2.Edit2.Text);//$
            form2.Edit2.SetFocus;
            form2.Edit2.SelStart:=utf8length(form2.Edit2.Text);
            form2.Edit2.SelLength:=0;
           end;
...
begin
  ...
  If form2.CheckBox1.Checked=false then form2.Bevel1.Visible:=false
     ...
     form2.Bevel1.Shape:=bsBox;
  checkstate:=false;
end;

procedure TForm2.CheckBox2Change(Sender: TObject);
begin
If dontcheck then exit;
  form2.inetdeny_ins();
end;

procedure TForm2.CheckBox3Change(Sender: TObject);
begin
  If checkbox3.Checked then form2.Shape1.Brush.Color:=clYellow
  else form2.Shape1.Brush.Color:=clDefault;
If dontcheck then exit;
  //внести признак необходимости внесения фио пассажира на заказном рейсе
 form2.fiomust_ins();
end;
...
//ввод серии и номера документа   водитель 2
...
begin
 ...
 //цифры - отбой
 //showmessage(inttostr(key));
  If (get_type_char(key)=1) then key:=0;

  //кавычки, точки, запятые, слэши
  If ((key>32) and (key<46) and not((key=37) or (key=39))) or ((key>57) and (key<63)) or ((key>90) and (key<97)) then
   key:=0;

  If key=0 then exit;
  flchange:=true;

  //writelog('Form2.Edit2KeyDown TEdit(Sender).SelText:  '+TEdit(Sender).SelText);//$
 //DELETE
   ...
   form2.ZReadOnlyQuery1.First;
     for n:=0 to form2.ZReadOnlyQuery1.RecordCount-1 do
       begin
         ...
         //ищем текущий атс в массиве
        if not findatp and (trim(form2.ZReadOnlyQuery1.FieldByName('id_kontr').asString)=kontr_id) and (tmp_ats[length(tmp_ats)-1,1]=ats_id)  then
           begin
             findatp:=true;
             tek_mas_ats:=length(tmp_ats)-1;
             fill_ats(tek_mas_ats);//заполнить переменные и поле АТС
...
begin
 ...
 If full_mas[idx,16]='1' then
  ...
  //id перевозчика
   ...
   nfirstmest :=0;
   try
   nfirstmest := strtoint(ats_mest);
   except
      on exception: EConvertError do
            showmessagealt('Ошибка конвертации числа мест !'+#13+'--err35');
   end;
 ...
 //form2.Label48.Caption:=ats_mest;
   ...
   //если незаявленное ТС
   If (utf8pos('|',full_mas[idx,33])>0) and (oper<>11) then
    begin
      checkstate:=true;
 ...
 if (trim(full_mas[idx,0])='2') or (trim(full_mas[idx,0])='4')
    //or (trim(full_mas[idx,0])='5')
    then
 begin
   zkaz:=true;
   SetLength(tmp_atp,0,0);
...
//Выравниваем форму
 ...
 form2.GroupBox3.Height:=65;
  ...
  dontcheck:=true;
  If inetdeny() then form2.CheckBox2.Checked:=false
  else CheckBox2.Checked:=true;
  IF (operation=11) then form2.CheckBox3.Enabled:=true;
  If fiomust() then form2.CheckBox3.Checked:=true; //проверка на необходимость внесения ФИО на заказных рейсах
  dontcheck:=false;
