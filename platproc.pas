...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls,Grids, Buttons,Forms,IniPropStorage, ComCtrls,LazUTF8,TypInfo,
  //{$IFDEF UNIX}
  //unix,
  //libc,
   //{$ENDIF}
...
//procedure SetMenuGridLocal(Grid:TStringGrid;user_id,user_id_arm:Integer;MyQuery:TZReadOnlyQuery); //Заполнение и доступы к меню LOCAL(начальная загрузка)
...
procedure ShowEditLog(aForm: TForm; Panel: TPanel; ZQ: TZReadOnlyQuery; ZCon: TZconnection;TableName: string; idnum: string; flag:Byte);// Вывод информации по созданию и редактированию записи
...
function in_user_sql(arm,appname,user,username,ip_adr:string;ZCon:TZConnection;ZQuery:TZReadOnlyQuery):byte; // Процедура проверки ВХОДА пользователя
function out_user_sql(ZCon:TZConnection;ZQuery:TZReadOnlyQuery):byte; // Процедура проверки ВЫХОДА пользователя
//function active_user_sql(ZCon:TZConnection; ZQuery:TZReadOnlyQuery; user:string):byte; // Обновление активных файлов пользователя
procedure grid_active_user(Grid:TStringGrid;ZCon:TZConnection;ZQuery:TZReadOnlyQuery); //Вывод в GRID активных пользователей
function DelCheck(Grid:TStringGrid; aCol:integer; ZCon:TZConnection; ZQ:TZReadOnlyQuery; sStables:string):byte; //ограничение целостности данных при удалении из таблиц
...
function Connect2(ZCon:TZConnection; mode: byte):boolean;// ПОПЫТКИ ПОДКЛЮЧЕНИЯ К БАЗЕ
...
function list_Servers(ZCon:TZConnection; ZQuery:TZReadOnlyQuery;idpoint:integer;user_id:integer):boolean; //Возвращает список серверов доступных для данного
...
procedure closeAll();//закрыть все формы
function Dialogmess(s:string):integer;// Альтернативный DialogMessage
function JustDigits(str :string): boolean;  // Проверяем что в строке только числа
procedure writelog(s:string); //запись лога
...
var
  ...
  Tek_server,Real_server, spb:integer;  //Текущий и локальный сервер
...
//остановочного пункта и текущего пользователя
function list_Servers(ZCon:TZConnection; ZQuery:TZReadOnlyQuery;idpoint:integer;user_id:integer):boolean;
 var
     n:integer;
begin
  ...
  // Определяем текущий по умолчанию сервер
     try
        Real_Server:=strtoint(mas_otkuda[0,0]);
         ...
         begin
          showmessagealt('ОШИБКА КОНВЕРТАЦИИ !!!'+#13+'НЕВЕРНОЕ ЗНАЧЕНИЕ ID Сервера !');
          exit;
         end;
      end;
...
begin
      ...
      if FileExistsUTF8(inipath) then
        begin
            ...
            IniObj.IniSection:='SETTINGS POGODA'; //указываем секцию
            Sale_INI[3]:=IniObj.ReadString('active','');
            Sale_INI[4]:=IniObj.ReadString('ip_proxy','');
            Sale_INI[5]:=IniObj.ReadString('port_proxy','');
            Sale_INI[6]:=IniObj.ReadString('login_proxy','');
            Sale_INI[7]:=IniObj.ReadString('password_proxy','');
        ...
        begin
           ...
           IniObj.IniSection:='SETTINGS POGODA'; //указываем секцию
           IniObj.WriteString('active','');
           IniObj.WriteString('ip_proxy','');
           IniObj.WriteString('port_proxy','');
           IniObj.WriteString('login_proxy','');
           IniObj.WriteString('password_proxy','');
...
function Connect2(ZCon:TZConnection; mode: byte):boolean;
//mode: тип подключения
//1 : может быть как локальный сервер, так и удалека
//2 : подключение только к локальному серверу
//3 : подключение только к удаленному серверу
...
begin
  ...
  Zcon.user:=cUser;
  Zcon.password:=cPass;
//если не определен сервер удаленки и подключение общее, то подключаемся к локалке
  If (mode=1) and ((trim(sale_server)='') or (trim(sale_server)='0')) then mode:=2;
  //если только к удаленке и не определен сервер удаленки - ошибка
  If (mode=3) and ((trim(sale_server)='') or (trim(sale_server)='0')) then
   begin
     showmessagealt('НЕ ОПРЕДЕЛЕН сервер УДАЛЕННОЙ продажи !');
     exit;
   end;

 //если подлкючение только к локальному серверу
 If mode=2 then
   begin
  ...
  if flagProfile=4 then
    ...
    //Подключение к удаленному серверу (можно и к локальному)
  if mode<>2 then
     begin
      flfind:=false;
      Zcon.user:='';
      ...
      Zcon.port:=0;
      Zcon.Database:='';
        // Переопределяем подключение на новый сервер
    for n:=0 to length(mas_otkuda)-1 do
   begin
        if trim(mas_otkuda[n,0])=sale_server then
         begin
         otkuda_name:=mas_otkuda[n,1];
          //showmessage(mas_otkuda[n,3]+#13+mas_otkuda[n,7]+#13+mas_otkuda[n,4]+#13+mas_otkuda[n,5]+#13);//$
        Zcon.hostname:=mas_otkuda[n,3];
     try
        Zcon.port:=strToInt(mas_otkuda[n,7]);
      except
         on exception: EConvertError do
         begin
          showmessagealt('ОШИБКА КОНВЕРТАЦИИ !!!'+#13+'НЕВЕРНОЕ ЗНАЧЕНИЕ ПОРТА !');
      ...
      //showmessage(mas_otkuda[n,4]);//$
     Zcon.Database:=mas_otkuda[n,4];
     Zcon.user:=mas_otkuda[n,5];
     Zcon.password:=mas_otkuda[n,6];
      flfind:=true;
         break;
        end;
   end;
    If not flfind then
     begin
      showmessagealt('Не определены параметры подключения !');
      exit;
     end;
    //showmessagealt(Zcon.user+#13+Zcon.password+#13+Zcon.hostname+#13+inttostr(Zcon.port)+#13+Zcon.Database);
  end;
   ...
   Zcon.Properties.Text:='timeout=8';
   try
      ZCon.connect;
   except
      on E:Exception do
  ...
  //  FormIdle.Show;
  //  If flProfile=5 then
  //  info := 'Подключение к серверу '+utf8copy(mas_otkuda[n,1],1,15)+'...'
  //  else
  //    info := 'Подключение к серверу '+Zcon.hostname+'...';
  //  flclose:=false;
  //  for i:=1 to cNCon do
  //   begin
  ...
  for n:=0 to length(mas_slow)-1 do
    begin
      ...
      case align_h of
        ...
        3:xc:=gRect.Left+(gRect.Right-w-4);
      end;
      case align_v of
        1:yc:=gRect.top+(n*h);
        2:yc:=gRect.top+((gRect.bottom-gRect.top) div 2)-(h div 2);
        3:yc:=(gRect.bottom-(h*(n+1))-4);
//        gRect.top+
      end;
...
begin
   cislo:=trim(StringReplace(cislo,' ','',[rfReplaceAll]));
...
function DelCheck(Grid:TStringGrid; aCol:integer; ZCon:TZConnection; ZQ:TZReadOnlyQuery; sStables:string):byte;
var
...
procedure grid_active_user(Grid:TStringGrid;ZCon:TZConnection;ZQuery:TZReadOnlyQuery);
 var
...
function active_user_sql(ZCon:TZConnection;ZQuery:TZReadOnlyQuery; user:string):byte;
var
...
function out_user_sql(ZCon:TZConnection;ZQuery:TZReadOnlyQuery):byte;
begin
  //--------------------------Пишем свой ID---------------------------------------
   // Подключаемся к серверу
   If not(Connect2(Zcon, 1)) then
...
function in_user_sql(arm,appname,user,username,ip_adr:string;ZCon:TZConnection;ZQuery:TZReadOnlyQuery):byte;
 var
  tek_id:integer;
begin
  //--------------------------Пишем свой ID---------------------------------------
   ...
   if ZQuery.RecordCount>0 then
      begin
           ...
           ZQuery.SQL.Add(' ORDER BY id DESC LIMIT 1;');
           try
              ZQuery.Open;
...
procedure ShowEditLog(aForm: TForm; Panel: TPanel; ZQ: TZReadOnlyQuery; ZCon: TZconnection;TableName: string; idnum: string; flag:Byte);
var
...
//procedure SetMenuGridLocal(Grid:TStringGrid;user_id,user_id_arm:Integer;MyQuery:TZReadOnlyQuery);
// var
//   n:integer;
//begin
...
// Альтернативный DialogMessage
function Dialogmess(s:string):integer;
 var
  i:integer;
  MyMesDlg:TForm;
  //MyComponent:TButton;
...
begin
  ...
  form10.Label2.Caption:=s;
  //form10.Left:=form1.Left+(form1.Width div 2)-(form10.Width div 2);
  //form10.top:=form1.Top+(form1.Height div 2)-(form10.height div 2);
  form10.BitBtn1.Visible:=true;
  form10.BitBtn2.Visible:=true;
  form10.Shape1.Visible:=true;
  form10.Shape2.Visible:=true;
  Form10.ShowModal;
  //showmessage(inttostr(sdd)+#13+inttostr(spb));//$
  Result:=spb;
  //FreeAndNil(Form10);
end;
...
begin
  If (trim(s)='') then exit;
  //если не исключение, писать файл лога
  If (utf8copy(s,1,1)='[') or (utf8copy(s,1,1)='.') then exit;

 namelog:=ExtractFilePath(Application.ExeName)+'disp_log/'+trim(FormatDateTime('yy-mm-dd', now())+'.log');
  // --------Проверяем что уже есть каталог LOG если нет то создаем
  If Not  DirectoryExistsUTF8(ExtractFilePath(Application.ExeName)+'disp_log') then
    begin
  ...
  if fileexistsUTF8(namelog) then Append(log_file) else Rewrite(log_file); // открытие файла для чтения
  {$I+} // включение контроля ошибок ввода-вывода
  if IOResult<>0 then // если есть ошибка открытия, то
   begin
     CloseFile(log_file);//изменено 28.05.2019
     Exit;
   end;
  ...
  // If screen.FormCount>1 then
   ...
   //writeln(log_file,'len full_mas: '+inttostr(length(full_mas))+' | '+'masindex: '+inttostr(masindex)+' | '+'zakaz: '+full_mas[masindex,0]+' | '+'shed: '+full_mas[masindex,1]+' | ')
   //else   writeln(log_file,'masindex: '+inttostr(masindex)+' | ');
   writeln(log_file,'sale_server: '+sale_server+' | '+'otkuda_name: '+otkuda_name+' | '+'opeation:'+inttostr(operation)+' | ');
   writeln(log_file,'======================================== СООБЩЕНИЕ ============================================');
   writeln(log_file,s);
   //writeln(log_file,'===============================================================================================');
   writeln(log_file,'|_                                                                                       _|');
